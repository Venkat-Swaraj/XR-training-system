...
namespace OrkestraLib
{
    ...
    public class Orkestra : MonoBehaviour, IOrkestra
    {
        ...
        [ObsoleteAttribute("This method has been deprecated. Use Dispatch(Channel.Application, ...) instead.", false)]
        public void SetAppAttribute(string key, string value)
        {
            AppContext.SetItem(key, value);
        }

        [ObsoleteAttribute("This method has been deprecated. Use Dispatch(Channel.User, ...) instead.", false)]
        public void SetUserItem(string toAgentId, string key, string value)
        {
            if (agentId.Equals(toAgentId))
            {
                AppContext.SelfAgent.SetItem(key, value);
            }
            else
            {
                RemoteAgent agent = AppContext.GetAgent(toAgentId);
                if (agent == null) throw new ArgumentException("The user does not exist");
                agent.Api.SetItem(key, value);
            }
        }
